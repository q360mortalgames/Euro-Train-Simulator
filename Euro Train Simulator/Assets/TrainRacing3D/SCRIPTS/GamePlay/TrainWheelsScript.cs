...
public class TrainWheelsScript : MonoBehaviour
{
		...
		else if (transform.root.GetComponent<TrainCollisionMultiPlayerHandler> () && transform.root.GetComponent<TrainCollisionMultiPlayerHandler> ().isAI && GamePlayManager.Instance.mb_IsAIBreak) {
			GetComponent<Rigidbody> ().drag	= Mathf.Lerp (GetComponent<Rigidbody> ().drag, 3f, Time.deltaTime * f_DragSpeed);
			//Debug.Log ("-- Apply drag to AI wheels..");
		}
		else if (transform.root.GetComponent<TrainCollisionMultiPlayerHandler> () && !transform.root.GetComponent<TrainCollisionMultiPlayerHandler> ().isAI && GamePlayManager.Instance.mb_IsBreak) {
			GetComponent<Rigidbody> ().drag	= Mathf.Lerp (GetComponent<Rigidbody> ().drag, 3f, Time.deltaTime * f_DragSpeed);
