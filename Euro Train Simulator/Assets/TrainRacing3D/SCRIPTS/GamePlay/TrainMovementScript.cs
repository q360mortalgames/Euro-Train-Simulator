...
//			_EngineRigidBody.isKinematic	= ISKinematic;
	...
	float getSpeed{
		get{
			return (GetComponent<TrainCollisionMultiPlayerHandler>() && GetComponent<TrainCollisionMultiPlayerHandler>().isAI)? GamePlayManager.Instance.f_DiffInSpeed_AI :GamePlayManager.Instance.f_DiffInSpeed;
		}
		set{
			getSpeed = value;
		}
	...
	bool getBreak{
		get{
			return (GetComponent<TrainCollisionMultiPlayerHandler> () && GetComponent<TrainCollisionMultiPlayerHandler> ().isAI) ? GamePlayManager.Instance.mb_IsAIBreak : GamePlayManager.Instance.mb_IsBreak;
		}
		set{
			getBreak = value;
		}
